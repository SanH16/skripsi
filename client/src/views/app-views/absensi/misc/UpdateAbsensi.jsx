...
export default function UpdateAbsensi({ onClose, refetchAbsensi, updateData }) {
  ...
  useEffect(() => {
    if (updateData) {
      setValue(
        "jam_masuk",
        updateData.jam_masuk ? dayjs(updateData.jam_masuk) : null,
      );
      setValue(
        "jam_keluar",
        updateData.jam_keluar ? dayjs(updateData.jam_keluar) : null,
      );
      setValue("status", updateData.status);
      setValue("keterangan", updateData.keterangan);
      setImagePreview(`http://localhost:5000/images/${updateData.bukti_photo}`);
    }
  }, [updateData, setValue]);

  const updateAbsensi = async () => {
    try {
      await APIabsensi.updateAbsensi(updateData.uuid, inputData);
      showSuccessToast(
        "Absensi Kehadiran berhasil diubah",
        "top-center",
        "large",
      );
      refetchAbsensi();
      ...
      console.error(err);
      showErrorToast("Absensi Kehadiran gagal diubah", "top-center", "large");
    }
  };
    ...
    <section id="unggah-absensi" className="mb-5 py-5">
      ...
      {isShowConfirm && (
        <ModalConfirm
          ...
          modalTitle="Update Absensi Kehadiran"
          inputData={inputData}
          action={updateAbsensi}
        >
          <>
            <p>Apakah anda yakin ingin mengubah data absensi?</p>
          </>
        </ModalConfirm>
      )}
    </section>
