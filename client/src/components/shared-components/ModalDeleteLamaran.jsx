...
export function ModalDeleteLamaran({ closeModal, stateModal, refetchDelete }) {
  const [isOpen, setIsOpen] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const handleOk = async () => {
    try {
      await APIlamaran.deleteLamaran(stateModal.uuid).then(() => {
        setIsOpen(false);
      ...
      console.error(error);
      showErrorToast("Lamaran gagal dihapus", "top-center", "large");
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h5 className="font-bold">Menghapus Data Pelamar</h5>
        <p
          id="logout-modal-text"
