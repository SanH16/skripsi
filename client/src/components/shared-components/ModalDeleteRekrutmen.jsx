...
export function ModalDeleteRekrutmen({ closeModal, detailRekrutmen }) {
  const [isOpen, setIsOpen] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const handleOk = async () => {
    try {
      await APIrekrutmen.deleteRekrutmen(detailRekrutmen.uuid).then(() => {
        setIsOpen(false);
      ...
      console.error(error);
      showErrorToast("Lowongan gagal dihapus", "top-center", "large");
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h5 className="font-bold">Menghapus Lowongan</h5>
        <p
          id="logout-modal-text"
          className="mt-2 text-sm leading-relaxed text-grey-400 sm:text-base md:px-3"
        >
          Lowongan ini akan dihapus secara permanen. Apakah anda yakin akan
          menghapusnya?
        </p>
      </div>
    </Modal>
