...
export function ModalDeletePegawai({ closeModal, stateModal, refetchDelete }) {
  ...
  const handleOk = async () => {
    try {
      await APIpegawai.deletePegawai(stateModal.uuid).then(() => {
        setIsOpen(false);
      ...
      console.error(error);
      showErrorToast("Data pegawai gagal dihapus", "top-center", "large");
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h5 className="font-bold">Menghapus Data Pegawai</h5>
        <p
          id="logout-modal-text"
