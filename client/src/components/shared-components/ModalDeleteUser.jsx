...
export function ModalDeleteUser({ closeModal, stateModal }) {
  ...
  const handleOk = async () => {
    try {
      await APIuser.deleteUser(stateModal.uuid).then(() => {
        setIsOpen(false);
      ...
      console.error(error);
      showErrorToast("Akun User gagal dihapus", "top-center", "large");
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h5 className="font-bold">Menghapus Akun User</h5>
        <p
          id="logout-modal-text"
          className="mt-2 text-sm leading-relaxed text-grey-400 sm:text-base md:px-3"
        >
          Akun ini akan dihapus secara permanen. Apakah anda yakin akan
          menghapusnya?
        </p>
      </div>
    </Modal>
