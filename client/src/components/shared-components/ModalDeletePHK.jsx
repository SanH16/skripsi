...
export function ModalDeletePHK({ closeModal, stateModal, refetchDelete }) {
  const [isOpen, setIsOpen] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const handleOk = async () => {
    try {
      await APIphk.deletePhk(stateModal.uuid).then(() => {
        setIsOpen(false);
        setIsSubmitting(true);
        closeModal();
        globalRoute.navigate("/pemutusan-hubungan-kerja");
        refetchDelete();
      });
    } catch (error) {
      console.error(error);
      showErrorToast("Data PHK gagal dihapus", "top-center", "large");
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h5 className="font-bold">Menghapus Data PHK</h5>
        <p
          id="logout-modal-text"
