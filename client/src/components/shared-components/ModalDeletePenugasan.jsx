...
export function ModalDeletePenugasan({
  closeModal,
  stateModal,
  refetchDelete,
}) {
  ...
  const handleOk = async () => {
    try {
      await APIpenugasan.deletePenugasan(stateModal.uuid).then(() => {
        setIsOpen(false);
      ...
      console.error(error);
      showErrorToast("Data Penugasan gagal dihapus", "top-center", "large");
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h5 className="font-bold">Menghapus Data Tugas</h5>
        <p
          id="logout-modal-text"
